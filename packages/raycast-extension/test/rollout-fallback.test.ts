...
const fixtureNames = ['rollout-user-japanese.jsonl', 'rollout-exec.jsonl', 'rollout-json-source.jsonl', 'rollout-subagent.jsonl'];
...
describe('scanRecentRollouts', () => {
  ...
  it('matches the SQL interactive source and thread-source exclusions', async () => {
    ...
    expect(rows.every((row) => row.source !== 'exec' && !row.source.startsWith('{') && row.thread_source !== 'subagent')).toBe(
      true,
    );
  });
